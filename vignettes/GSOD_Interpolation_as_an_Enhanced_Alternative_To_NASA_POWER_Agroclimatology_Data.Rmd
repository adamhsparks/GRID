...
years <- as.list(seq(from = 1983, to = 2010, by = 1))
...
to run the function and save GTiff files in "~/Cache/GTiff" using threeq cores.

```{r interpolate_2_cores,  eval=FALSE, message='hide', results='hide'}
GRID <- lapply(X = file_list, FUN = interpolate_GSOD, dem = DEM,
               dsn = "/mnt/Cache/GTiff", vars = c("MAX", "MIN"), cores = 3)
```
