...
```{r get_GSOD, eval=TRUE, message='hide', results='hide'}
...
```{r get_DEM, eval=TRUE, message='hide', results='hide'}
DEM <- make_DEM(dsn = "/mnt/DEM")
```

### Step 3: Interpolate the GSOD Data from On-disk Files

```{r list_GSOD_files, eval=TRUE, message='hide', results='hide'}
...
```{r interpolate,  eval=TRUE, message='hide', results='hide'}
GRID <- lapply(X = file_list, FUN = interpolate_GSOD, dem = DEM,
               dsn = "/mnt/Cache/GTiff", vars = c("TMAX", "TMIN"))
```
